...
async def fetch_admins_from_api(chat_id: str) -> list:
    """Получает список администраторов из MAX API"""
...
def get_message_info(event: MessageCreated):
    ...
    try:
        ...
        # ✅ КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: используем body.mid
        msg_id = None
...
async def check_message(event: MessageCreated):
    ...
    if user_id in admin_ids:
        logger.info(f"✅ {user_id} — админ, сообщение оставляем")
        return
    
    # Удаляем сообщение
    logger.info(f"🚫 {user_id} НЕ админ, удаляем сообщение {msg_id}")
...
async def handle_help(event: MessageCreated):
    """Команда /help"""
    _, _, _, text = get_message_info(event)
    if text == '/help':
...
async def handle_refresh(event: MessageCreated):
    ...
    if text == '/refresh':
        ...
        if user_id in admin_ids:
            ...
            await send_message(
                chat_id,
                f"🔄 Список админов обновлён!\nНайдено: {len(new_admins)} админов"
            )
        else:
            await send_message(chat_id, "❌ Только администраторы могут обновлять список")
